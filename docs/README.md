# Documentation

Supplementary documentation for the Security Verification Toolkit.

## Contents

| Document | Description |
|----------|-------------|
| [AIRGAPPED.md](AIRGAPPED.md) | Guide for air-gapped/offline deployments |
| [ASSESSOR-QUALIFICATIONS.md](ASSESSOR-QUALIFICATIONS.md) | Recommended certifications for security assessors |
| [BASH-SET-E-PITFALLS.md](BASH-SET-E-PITFALLS.md) | `set -e` and arithmetic pitfalls (required reading for contributors) |
| [BRANCH-PROTECTION.md](BRANCH-PROTECTION.md) | GitHub branch protection configuration |
| [DEMO-CHEAT-SHEET.md](DEMO-CHEAT-SHEET.md) | Demo walkthrough: Kali → Ubuntu SSH remote scan |
| [COMPLIANCE.md](COMPLIANCE.md) | NIST control mapping and compliance workflow |
| [DEPENDENCIES.md](DEPENDENCIES.md) | Toolkit dependency requirements and versions |
| [EXECUTIVE-BRIEF.md](EXECUTIVE-BRIEF.md) | High-level overview for stakeholders |
| [FAQ.md](FAQ.md) | Frequently asked questions |
| [FALSE-POSITIVES-MACOS.md](FALSE-POSITIVES-MACOS.md) | macOS-specific false positive guidance |
| [LIBRARY-API.md](LIBRARY-API.md) | Shell library API reference for contributors |
| [MAINTENANCE.md](MAINTENANCE.md) | Maintenance schedules and update procedures |
| [MALWARE-SCANNER-ABSTRACTION.md](MALWARE-SCANNER-ABSTRACTION.md) | Malware scanner abstraction proposal |
| [PERFORMANCE.md](PERFORMANCE.md) | Performance optimization and benchmarks |
| [SSH-TROUBLESHOOTING.md](SSH-TROUBLESHOOTING.md) | SSH troubleshooting for remote scans |
| [TESTING.md](TESTING.md) | Test architecture and contributor guide |
| [THREAT-INTELLIGENCE.md](THREAT-INTELLIGENCE.md) | CISA KEV, DHS MARs, NASA SOC-MARs integration |
| [TROUBLESHOOTING.md](TROUBLESHOOTING.md) | Advanced diagnostics and troubleshooting |
| [WINDOWS-DEVELOPMENT-PLAN.md](WINDOWS-DEVELOPMENT-PLAN.md) | Windows PowerShell development roadmap |
| [WINDOWS-TARGET-SETUP.md](WINDOWS-TARGET-SETUP.md) | Windows target setup for remote scanning |

## Core Documentation

For primary documentation, see the repository root:

- [README.md](../README.md) - Quick start and usage
- [INSTALLATION.md](../INSTALLATION.md) - Platform-specific setup
- [CLAUDE.md](../CLAUDE.md) - AI agent instructions and architecture
- [SECURITY.md](../SECURITY.md) - Vulnerability reporting
- [CHANGELOG.md](../CHANGELOG.md) - Version history

## Related Directories

| Directory | Purpose |
|-----------|---------|
| [requirements/](../requirements/) | Machine-readable requirements and NIST control mappings |
| [verification/](../verification/) | Verification evidence and templates |
| [templates/](../templates/) | LaTeX templates for PDF generation |
| [examples/](../examples/) | Redacted example scan outputs |
