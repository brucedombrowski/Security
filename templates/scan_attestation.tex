...
\newcommand{\resultcell}[1]{\ifnum\pdfstrcmp{#1}{PASS}=0 \passmark\else\ifnum\pdfstrcmp{#1}{EXCEPT}=0 \passexceptmark\else\failmark\fi\fi}
...
Items flagged by automated scans but reviewed and accepted as non-issues are documented in the PII allowlist file. Each exception includes a SHA256 hash for integrity verification and reviewer justification.

\vspace{6pt}
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Allowlist File} & \texttt{.pii-allowlist} \\
\textbf{SHA256 (first 16)} & \texttt{\AllowlistChecksum} \\
\textbf{Total Exceptions} & \AllowlistCount \\
\bottomrule
\end{tabular}

\vspace{6pt}
\textbf{Reviewed Justifications:}
...
\vspace{6pt}
{\small\textbf{Note:} Full details including file paths and SHA256 hashes are in \texttt{.pii-allowlist}.}
...
\vspace{6pt}
Full file names: \texttt{*-\ScanFileTimestamp.txt}. Complete checksums in \texttt{.scans/checksums.md}.
